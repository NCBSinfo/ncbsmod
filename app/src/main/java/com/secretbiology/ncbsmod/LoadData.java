...
public class LoadData extends AppCompatActivity implements EasyPermissions.PermissionCallbacks{
            ...
            private List<String> getDataFromApi() throws IOException {
                ...
                if (values != null) {
                    for (List row : values) {
                        ...
                        ExternalDataModel entry = new ExternalDataModel();
                        entry.setTimestamp(row.get(0).toString());
                        entry.setName(row.get(1).toString());
                        entry.setEmail(row.get(2).toString().replaceAll("\\s+",""));
                        entry.setToken(row.get(3).toString());
                ...
                if (mLastError != null) {
                    if (mLastError instanceof GooglePlayServicesAvailabilityIOException) {
                        ...
                        startActivityForResult(
                                ...
                                LoadData.REQUEST_AUTHORIZATION);
                    } else {
                        Log.i("Tag","The following error occurred:\n"
                                + mLastError.getMessage());

                        final AlertDialog alertDialog = new AlertDialog.Builder(
                                LoadData.this).create();
                        if(mLastError.getMessage().contains("PERMISSION_DENIED")){
                            alertDialog.setTitle("Oops!");
                            alertDialog.setMessage("You don't have access to moderator's zone!");
                            alertDialog.setButton(DialogInterface.BUTTON_POSITIVE, "OK", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    alertDialog.dismiss();
                                    startActivity(new Intent(LoadData.this, Home.class));
                                }
                            });
